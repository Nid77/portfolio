<template>
    <section v-if="projets != undefined && projets.length > 0" class="flex anim-scale-up-hor-left">
        <h2>{{ title }}</h2>
        <div class="flex flex-row w-full items-center justify-center">
            <RouterLink v-for="(p, index) in projets" :key="p.nom" :to="{ name: 'project', params: { id: p.nom } }"
                :class="['anim-slide-in-right', `delay-${1 + index}`]">
